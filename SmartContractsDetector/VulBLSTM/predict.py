...
def mainpredict():
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            checkpoint_file = tf.train.latest_checkpoint("differentmodels/600model")
            saver = tf.train.import_meta_graph("{}.meta".format(checkpoint_file))
            saver.restore(sess, checkpoint_file)
            #print("print all Tensor",[n.name for n in tf.get_default_graph().as_graph_def().node])

            inputX = graph.get_operation_by_name("inputX").outputs[0]
            inputY = graph.get_operation_by_name("inputY").outputs[0]
            dropoutKeepProb = graph.get_operation_by_name("dropoutKeepProb").outputs[0]

            pred_all = graph.get_tensor_by_name("output/predictions:0")
            pred_binay = graph.get_tensor_by_name("output/binaryPreds:0")
            for batchEval in norandomnextBatch(inferenceReviews, inferenceLabels,config.testSize):
                ...
                pred_all_binary = sess.run(pred_binay,feed_dict={inputX: batchEval[0],
                                                          inputY: batchEval[1],
                                                          dropoutKeepProb: 1})
                print("pred_all",pred_all)
                labels.append(pred_all_binary)
                print("pred_binary",pred_binay)
                print("pred_all_1",pred_all_binary)
            print("labels",labels)
