...
with open("rawData/labeledunVulTrainData.tsv", "r",encoding='UTF-8') as f:
    unlabeledTrain = [line.strip().split("\t") for line in f.readlines() if len(line.strip().split("\t")) == 2]

with open("rawData/labeledVulTrainData.tsv", "r",encoding='UTF-8') as f:
...
newDf = pd.concat([unlabel["review"], label["review"]], axis=0)
    # 保存成txt文件
newDf.to_csv("rawData/wordEmbdiing.txt", index=False)